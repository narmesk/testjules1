...
begin
  ...
  try
    ...
    begin
      ...
      Grid.Cells[4, i + 1] := FormatFloat('0.0', DataArray[i].BPozisyon / 100) + '\xb0';
      ...
      Grid.Cells[10, i + 1] := FormatFloat('0.0', DataArray[i].DPozisyon / 100) + '\xb0';
...
begin
  ...
  begin
    if (EditValue < -9999.9) or (EditValue > 9999.9) then (Sender as TJvEdit).Color := clRed
    else (Sender as TJvEdit).Color := clWhite;
  end else (Sender as TJvEdit).Color := clYellow;
end;
...
begin
  ...
  if TryStrToFloat((Sender as TJvEdit).Text, EditValue) then
  begin
    if (EditValue < 0) or (EditValue > 3000) then (Sender as TJvEdit).Color := clRed
    else (Sender as TJvEdit).Color := clWhite;
  end else (Sender as TJvEdit).Color := clYellow;
end;
...
begin
  ...
  if TryStrToFloat((Sender as TJvEdit).Text, EditValue) then
  begin
    if (EditValue < -360.0) or (EditValue > 360.0) then (Sender as TJvEdit).Color := clRed
    else (Sender as TJvEdit).Color := clWhite;
  end else (Sender as TJvEdit).Color := clYellow;
end;

procedure TDiagnose.JvEdit4Change(Sender: TObject); begin JvEdit2Change(Sender); end;
procedure TDiagnose.JvEdit5Change(Sender: TObject); begin JvEdit1Change(Sender); end;
procedure TDiagnose.JvEdit6Change(Sender: TObject); begin JvEdit2Change(Sender); end;
procedure TDiagnose.JvEdit7Change(Sender: TObject); begin JvEdit3Change(Sender); end;
procedure TDiagnose.JvEdit8Change(Sender: TObject); begin JvEdit2Change(Sender); end;
...
begin
  ...
  begin
    ...
    JvEdit3.Text := StringReplace(Recete.Cells[7, RowIndex], '\xb0', '', [rfReplaceAll, rfIgnoreCase]);
    ...
    JvEdit7.Text := StringReplace(Recete.Cells[17, RowIndex], '\xb0', '', [rfReplaceAll, rfIgnoreCase]);
    JvEdit8.Text := StringReplace(Recete.Cells[19, RowIndex], ' rpm', '', [rfReplaceAll, rfIgnoreCase]);

    DataArray[RowIndex - 1].AValf := (Recete.Cells[2, RowIndex] = 'ON');
    ...
    DataArray[RowIndex - 1].CValf := (Recete.Cells[11, RowIndex] = 'ON');
    ...
    DataArray[RowIndex - 1].DValf := (Recete.Cells[16, RowIndex] = 'ON');
...
begin
  ...
  ProcessCheck.Enabled := False;
  if JvOpenDialog1.Execute then SetSelectedFile(JvOpenDialog1.FileName);
  ProcessCheck.Enabled := True;
end;

procedure TDiagnose.JvNavPanelButton2Click(Sender: TObject);
begin
  ALL_CMD_REG := HOME_POS_GO; AccumulateDPozisyon := 0; IslenenAdim := 0; OutVal := 0;
  LastProcess1 := False; LastPRocess := 0; First_State := 5; Pause := 3; AcilDurum := 0;
end;

procedure TDiagnose.JvNavPanelButton3Click(Sender: TObject);
begin
  ProcessCheck.Enabled := False; ButtonTimer.Enabled := False;
  if IslenenAdim = 0 then begin JvNavPanelButton11Click(Sender); IslenenAdim := 1; IOStringGrid1.Row := 2; end
  else begin ButtonTimer.Enabled := True; ButtonTimerTimer(Sender); end;
end;
...
begin
  ALL_CMD_REG := RESET_GO; AccumulateDPozisyon := 0; IslenenAdim := 0; OutVal := 0;
  LastProcess1 := False; LastPRocess := 0; First_State := 5; Pause := 3; AcilDurum := 0;
end;
...
begin
  ...
  if LastFilePath <> '' then begin SaveStructToJson(LastFilePath); SetSelectedFile(LastFilePath); ShowMessage('Dosya kaydedildi!'); end;
end;

procedure TDiagnose.JvNavPanelButton8Click(Sender: TObject);
var i: Integer;
begin
  Recete.RowCount := Recete.RowCount + 1; NewRowIndex := Recete.RowCount - 1;
  UpdateStepNumbers(Recete); SetLength(DataArray, Length(DataArray) + 1);
  Recete.Cells[4, NewRowIndex] := 'mm'; Recete.Cells[6, NewRowIndex] := 'rpm';
  Recete.Cells[8, NewRowIndex] := '\xb0'; Recete.Cells[10, NewRowIndex] := 'rpm';
  Recete.Cells[13, NewRowIndex] := 'mm'; Recete.Cells[15, NewRowIndex] := 'rpm';
  Recete.Cells[18, NewRowIndex] := '\xb0'; Recete.Cells[20, NewRowIndex] := 'rpm';
  Recete.Row := NewRowIndex; Recete.OnClick(Self); Trace('Yeni sat\xfdr eklendi!');
end;
...
begin
  ...
  if RowIndex > 0 then begin
    for i := RowIndex to Recete.RowCount - 2 do begin
      Recete.Rows[i].Assign(Recete.Rows[i + 1]);
      DataArray[i - 1] := DataArray[i];
    end;
    Recete.RowCount := Recete.RowCount - 1; SetLength(DataArray, Length(DataArray) - 1);
    UpdateStepNumbers(Recete); Trace('Sat\xfdr silindi!');
  end;
end;
...
begin
  LatencyTimer.Enabled := False; Diagnose.ProcessCheck.Enabled := False;
  if GetBit(LastPRocess, 0) then begin if JvNavPanelButton3.Enabled then begin IslenenAdim := 0; ReceteSatir0Run(); IslenenAdim := 1; end; end;
  LastPRocess := 0; Diagnose.ProcessCheck.Enabled := True;
end;

procedure TDiagnose.ProcessCheckTimer(Sender: TObject);
begin
  if First_State > 0 then First_State := First_State - 1;
  if First_State < 3 then begin
    if GetBit(InVal, 0) and not AutoMan then begin AutoMan := True; ALL_CMD_REG := AUTO_GO; Pause := 1; end
    else if not GetBit(InVal, 0) and AutoMan then begin AutoMan := False; ALL_CMD_REG := MANUEL_GO; end;
    if GetBit(InVal, 7) then Kontaktor := True else Kontaktor := False;
    if (Kontaktor = True) AND (ReadyStatX = True) AND (ReadyStatY = True) AND (ReadyStatZ = True) AND (ReadyStatA = True) then
      SystemReady := True else SystemReady := False;
...
begin
  RowIndex := Recete.Row;
  if RowIndex > 0 then begin
    JvSwitch1.StateOn := (Recete.Cells[2, RowIndex] = 'ON');
    JvEdit1.Text := Recete.Cells[3, RowIndex]; JvEdit2.Text := Recete.Cells[5, RowIndex];
    JvEdit3.Text := Recete.Cells[7, RowIndex]; JvEdit4.Text := Recete.Cells[9, RowIndex];
    JvSwitch2.StateOn := (Recete.Cells[11, RowIndex] = 'ON');
    JvEdit5.Text := Recete.Cells[12, RowIndex]; JvEdit6.Text := Recete.Cells[14, RowIndex];
    JvSwitch3.StateOn := (Recete.Cells[16, RowIndex] = 'ON');
    JvEdit7.Text := Recete.Cells[17, RowIndex]; JvEdit8.Text := Recete.Cells[19, RowIndex];
  end;
end;

procedure TDiagnose.ReceteDblClick(Sender: TObject);
begin
  if Recete.Col in [2, 11, 16] then begin
    if Recete.Cells[Recete.Col, Recete.Row] = 'ON' then Recete.Cells[Recete.Col, Recete.Row] := 'OFF'
    else Recete.Cells[Recete.Col, Recete.Row] := 'ON';
...
var LGrid: TStringGrid;
begin
  LGrid := Sender as TStringGrid;
  if (ARow >= LGrid.FixedRows) and (ACol in [2, 11, 16]) then begin
    if LGrid.Cells[ACol, ARow] = 'ON' then LGrid.Canvas.Brush.Color := clGreen
    else LGrid.Canvas.Brush.Color := clRed;
  end else LGrid.Canvas.Brush.Color := clWhite;
  LGrid.Canvas.FillRect(Rect); LGrid.Canvas.TextOut(Rect.Left+2, Rect.Top+2, LGrid.Cells[ACol, ARow]);
end;

procedure TDiagnose.ReceteExitCell(Sender: TJvStringGrid; AColumn, ARow: Integer; const EditText: string);
begin JvNavPanelButton10Click(Sender); end;

procedure TDiagnose.ReceteKeyPress(Sender: TObject; var Key: Char);
begin if not (Key in ['0' .. '9', #8, ',', '-', '+', #13]) then Key := #0; end;

procedure TDiagnose.ReceteMouseDown(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin end;
...
begin
  Result := $FFFF;
  for i := Start_Adress to (Start_Adress + Count - 1) do begin
    bTemp := Word(Buffer[i]) xor Result;
    Result := (Result shr 8) xor CRC16Table[bTemp mod 256];
  end;
  Result := Swap(Result);
end;
...
function Enable_a_Bit(const aValue: Cardinal; const Bit: Byte; const Flag: Boolean): Cardinal; begin Result := (aValue or (1 shl Bit)) xor (Integer(not Flag) shl Bit); end;

procedure Check_Data();
var Idx: Integer; Buffer: TIDBytes; crc: Word; crcl, crch: Byte;
begin
  Protocol_UniIdx := Protocol_UniIdx + 1;
  case ALL_CMD_REG of
    HOME_POS_GO, VIRT_POS_GO, RESET_GO, AUTO_GO, MANUEL_GO: begin Diagnose.JvMemo1.Clear; ALL_STAT_REG := 0; end;
    else begin ALL_CMD_REG := 0; ALL_STAT_REG := 0; end;
  end;
  Motion_Data();
  crc := CalculateCRC16(TXMOTIONMEM, 0, 70);
  TXMOTIONMEM[70] := crc shr 8; TXMOTIONMEM[71] := crc mod 256;
  ...
  TXMEMORY[15] := OutVal mod 256; crc := CalculateCRC16(TXMEMORY, 0, 126);
  TXMEMORY[126] := crc shr 8; TXMEMORY[127] := crc mod 256; SetLength(Buffer, 128);
  for Idx := 0 to 127 do Buffer[Idx] := TXMEMORY[Idx];
  if not Diagnose.IdUDPClient1.Connected then begin
    try
      Diagnose.IdUDPClient1.Active := False; Diagnose.IdUDPClient1.Active := True;
      Diagnose.IdUDPClient1.SendBuffer(Buffer);
    except on E: Exception do Trace(E.Message); end;
    finally
      for Idx := 0 to 63 do Buffer[Idx] := $0;
      if Diagnose.IdUDPClient1.Binding.Readable(63) then begin
...
begin
  ...
  SystemReady := True; AutoMan := False; Kontaktor := False; DayamaDurum := 0;
  IslenenAdim := 0; First_State := 5; Pause := 3; EditTableClick(Sender);
  LastFilePath := Diagnose.ValueListEditor3.Values[Diagnose.ValueListEditor3.Keys[Diagnose.ValueListEditor3.RowCount - 1]];
  if LastFilePath <> '' then SetSelectedFile(LastFilePath);
  ProcessCheck.Enabled := True;
end;

procedure TDiagnose.Timer1Timer(Sender: TObject);
begin
  Edit5.Text := DateToStr(Now); Edit6.Text := TimeToStr(Now);
end;

procedure ActualStringGridInit();
begin
  Diagnose.IOStringGrid1.Cells[0, 0] := 'ADIM';
  Diagnose.Recete.Cells[1, 0] := 'ID'; Diagnose.Recete.Row := 1;
end;
...
procedure TDiagnose.FormCreate(Sender: TObject);
begin ReportMemoryLeaksOnShutdown := True; FormLoaded := False; Protocol_Create(); end;
