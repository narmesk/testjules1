...
begin
  ...
  try
    ...
    JSONFile := TStringList.Create;
    try
      JSONFile.Text := JSONArray.Format(2);
...
begin
  ...
  end else Trace('Lütfen düzenlenebilir bir satır seçiniz!');
end;
...
begin
  JvOpenDialog1.Filter := 'JSON Files (*.json)|*.json';
  ProcessCheck.Enabled := False;
  if JvOpenDialog1.Execute then begin SetSelectedFile(JvOpenDialog1.FileName); end;
...
begin
  ...
  Recete.Cells[8, NewRowIndex] := '°'; Recete.Cells[10, NewRowIndex] := 'rpm';
  Recete.Cells[13, NewRowIndex] := 'mm'; Recete.Cells[15, NewRowIndex] := 'rpm';
  Recete.Cells[18, NewRowIndex] := '°'; Recete.Cells[20, NewRowIndex] := 'rpm';
  Recete.Row := NewRowIndex; Recete.OnClick(Self); Trace('Yeni satır eklendi!');
end;
...
begin
  ...
  if RowIndex > 0 then begin
    ...
    UpdateStepNumbers(Recete); Trace('Satır silindi!');
  end;
end;
...
begin
  ...
  begin
    ...
    JvEdit2.Text := StringReplace(Recete.Cells[5, RowIndex], ' rpm', '',
      [rfReplaceAll, rfIgnoreCase]);
    JvEdit3.Text := StringReplace(Recete.Cells[7, RowIndex], '°', '',
      [rfReplaceAll, rfIgnoreCase]);
    ...
    JvEdit6.Text := StringReplace(Recete.Cells[14, RowIndex], ' rpm', '',
      [rfReplaceAll, rfIgnoreCase]);
    JvEdit7.Text := StringReplace(Recete.Cells[17, RowIndex], '°', '',
      [rfReplaceAll, rfIgnoreCase]);
    ...
    Trace('Lütfen düzenlenebilir bir satır seçiniz!');
end;
