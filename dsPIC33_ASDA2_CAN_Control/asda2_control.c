...
// KRİTİK: Her eksen kendi hardware buffer'ını kullanır (0, 1, 2, 3)
void Send_Axis_PDO(uint8_t axis_idx, int32_t target_pulse) {
    ...
    CAN1_TransmitToBuffer(axis_idx, &msg);
}
...
void Send_SYNC_Message(void) {
    ...
    CAN1_TransmitToBuffer(4, &sync_msg); // SYNC için Buffer 4
}
...
void CANopen_Start_Nodes(void) {
    ...
    data[0] = 0x01;
    data[1] = 0x00;
    CAN1_TransmitToBuffer(5, &nmt); // NMT için Buffer 5
}
